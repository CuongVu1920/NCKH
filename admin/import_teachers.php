...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    $default_role = 'giangvien';   // Vai trò mặc định

    foreach ($rows as $row) {
        // Nếu không có mã số GV thì bỏ qua
        ...
        $check_stmt->bind_param("s", $ma_gv);
        $check_stmt->execute();
        $check_result = $check_stmt->get_result();

        if ($check_result->num_rows > 0) {
            $errorCount++;
            $errors[] = "Mã số giảng viên $ma_gv đã tồn tại";
            continue;
        }
        ...
        // Insert giảng viên với mật khẩu và vai trò mặc định
        ...
        $stmt->bind_param("ssssssssss", $ma_gv, $ho_ten, $email, $default_password, $so_dien_thoai, $ngay_sinh, $gioi_tinh, $dia_chi, $id_chuyennganh, $default_role);

        if ($stmt->execute()) {
            $successCount++;
        } else {
            $errorCount++;
            $errors[] = "Lỗi khi thêm giảng viên $ma_gv: " . $stmt->error;
        }
    }

    $message = "Import hoàn tất: $successCount giảng viên được thêm thành công";
    if ($errorCount > 0) {
        $_SESSION['import_errors'] = $errors;
        header("Location: admin_dashboard.php?page_layout=teacher_list");
        exit();
    } else {
        header("Location: admin_dashboard.php?page_layout=teacher_list&message=" . urlencode($message));
        exit();
    }
} else {
    header("Location: admin_dashboard.php?page_layout=teacher_list");
